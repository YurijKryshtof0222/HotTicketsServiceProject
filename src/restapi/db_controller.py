...
class DbController:
    ...
    def get_filtered_records_as_json(self,
        ...
        conditions_dict["offer_name_condition"] = f'offer_name = "{offer_name}"' if offer_name and offer_name.strip() else ''
        conditions_dict["country_condition"] = f'location LIKE "{country}"%' if country and country.strip() else ''
        conditions_dict["people_count_condition"] = f' people_count {people_count_operation} {people_count_value}' \
            if (people_count_operation
                and people_count_operation.strip()
                and people_count_value > 0) else ''
        conditions_dict["food_info_condition"] = f'food_info = "{food_info}"' if (
                    food_info and food_info.strip()) else ''
        conditions_dict["night_count_condition"] = f'night_count {night_count_operation} {night_count_value}' \
            if (night_count_operation
                and night_count_operation.strip()
                and night_count_value > 0) else ''
        conditions_dict["end_date_condition"] = f'start_date {end_date_operation}, {end_date_value}' \
            if (end_date_operation
                and end_date_operation.strip()
                and end_date_value > 0) else ''
        conditions_dict["start_date_condition"] = f'start_date {start_date_operation}, {start_date_value}' \
            if (start_date_operation
                and start_date_operation.strip()
                and start_date_value > 0) else ''

        conditions_dict["price_condition"] = f'price {price_operation} {price_value}' if (price_operation
